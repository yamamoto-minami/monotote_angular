/**
 * a fork of https://github.com/DaftMonk/angular-tour
 */

(function (window, document, undefined) {
  'use strict';
  angular.module('cmp.tour', [
    'cmp.tour.tpls',
    'cmp.tour.tour'
  ]);
  angular.module('cmp.tour.tpls', []);
  angular.module('cmp.tour.tour', []).constant('tourConfig', {
    placement: 'top',
    animation: true,
    nextLabel: 'Next',
    scrollSpeed: 100,
    ...
    function ($parse, $timeout, tourConfig) {
      return {
        ...
        link: function (scope, element, attrs, ctrl) {
          ...
          attrs.onInit && scope.$eval(attrs.onInit, {
              $ctrl: ctrl
          });
        }
      };
  ...
  .directive('tourtipArrow', function(){
    var config = {
      restict: 'EA',
      require: '^tourPopup',
      link: link,
    ...
    function link(scope, element, attrs, tourtipCtrl) {
      tourtipCtrl.setArrowElement(element);
    }

    return config;
  }).controller('TourtipController', ['$scope',function($scope){
    this.setArrowElement = function(element){
      $scope.arrowElement = element;
    };
  }]).directive('tourtip', [
    '$window',
    '$compile',
    ...
    function ($window, $compile, $interpolate, $timeout, scrollTo, tourConfig, debounce) {
      ...
      return {
        ...
        scope: true,
        transclude: true,
        compile: function(e,a,transclude){ /* [compile] */return function (scope, element, attrs, tourCtrl) {
          ...
          attrs.$observe('tourtipPlacement', function (val) {
            ...
            scope.centered = /^center$/.test(scope.ttPlacement);
          });
          ...
          attrs.$observe('tourtipClass', function (val) {
            scope.ttMargin = parseInt(val, 10) || tourConfig.margin;
          });
          ...
          attrs.$observe('tourtipArrowOffsetVertical', function (val) {
            scope.arrowOffsetVertical = parseInt(val, 10) || 0;
          });
          attrs.$observe('tourtipArrowOffsetHorizontal', function (val) {
            scope.arrowOffsetHorizontal = parseInt(val, 10) || 0;
          });
          attrs.$observe('tourtipClass', function(val){
            scope.ttClass = val || '';
          })
          attrs.$observe('tourtipLeftButton', function (val) {
            scope.ttLButton = val || '';
          });
          attrs.$observe('tourtipRightButton', function (val) {
            scope.ttRButton = val || '';
          });
          attrs.$observe('tourtipLeftButtonText', function (val) {
            scope.ttLButtonText = val || '';
          });
          attrs.$observe('tourtipRightButtonText', function (val) {
            scope.ttRButtonText = val || '';
          });
          attrs.$observe('tourtipMinWidth', function(val) {
            scope.minWidth = parseInt(val, 10) || 0;
          });
          //defaults: null
          ...
          scope.ttClass = attrs.tourtipClass || '';
          ...
          scope.index = scope.$eval(attrs.tourtipStep);
          ...
          $timeout(function () {
            transclude(scope, function(elements){
              tourtip.find('[tour-content]').append(elements);
            });
            scope.$watch('ttOpen', function (val) {
              if (val) {
                var ttSkip = attrs.tourtipSkipIf && scope.$eval(attrs.tourtipSkipIf);
                if(ttSkip) {
                  var ttNextStep = attrs.tourtipSkipToStep && scope.$eval(attrs.tourtipSkipToStep);
                  if(!ttNextStep || ttNextStep === scope.getCurrentStep())
                    ttNextStep = scope.getCurrentStep()+1;
                  scope.setCurrentStep(ttNextStep);
                } 
                else show();
              } else {
                hide();
          ...
          function calculatePosition(element, container) {
            ...
            if(scope.minWidth && ttWidth < scope.minWidth)
              ttWidth = scope.minWidth;
            ...
            case 'right':
              ...
              break;
            case 'bottom':
              var _left = position.left - containerLeft + scope.offsetVertical;
              ttPosition = {
            ...
            case 'center':
              ...
              ttPosition = {
                top: Math.max(top + 0.5 * (position.height - ttHeight) + scope.ttMargin + scope.offsetVertical, 0),
                left: _left > 0 ? _left : minimumLeft
              };
              break;
            case 'center-top':
            ...
            case 'center-bottom':
              ...
              ttArrowPosition.left = position.left - ttPosition.left + position.width * 0.5 + scope.arrowOffsetHorizontal;
              break;
            case 'left':
            ...
            Object.keys(ttPosition).forEach(function(key){
              ttPosition[key] += 'px';
            });
            Object.keys(ttArrowPosition).forEach(function(key){
              ttArrowPosition[key] += 'px';
            });
            return {
              ttPosition: ttPosition,
              ttArrowPosition: ttArrowPosition
            };
          }
          function show() {
            ...
            setUpdateTimer();
            angular.element($window).bind('resize.'+scope.$id, setUpdateTimer);
            updatePosition(true);
            if (scope.onStepShow) {
          ...
          function updatePosition(scrollThePage) {
              var targetElement = scope.ttElement ? angular.element(scope.ttElement) : element;
              if (targetElement == null || targetElement.length === 0)
                throw 'Target element could not be found. Selector: ' + scope.ttElement;
              var offsetElement = scope.ttContainerElement === 'body' ? undefined : angular.element(scope.ttContainerElement);
              var positions = calculatePosition(targetElement, offsetElement);
              // Now set the calculated positioning.
              tourtip.css(positions.ttPosition);
              scope.arrowElement && scope.arrowElement.css(positions.ttArrowPosition);
              if(scrollThePage)
              {
                // Scroll to the tour tip
                var ttPositionTop = parseInt(positions.ttPosition.top), ttPositionLeft = parseInt(positions.ttPosition.left);
                scrollTo(tourtip, scope.ttContainerElement, -150, -300, tourConfig.scrollSpeed, ttPositionTop, ttPositionLeft);
              }
          }
          function setUpdateTimer(){
            cancelUpdateTimer();
            scope.updateTimer = $window.setInterval(updatePosition, 1000/20);
            scope.updateTimerTimeout = $window.setTimeout(function(){
              scope.updateTimerTimeout = null;
              cancelUpdateTimer();
            }, 700);
          }
          function cancelUpdateTimer(){
            scope.updateTimer && $window.clearInterval(scope.updateTimer);
            scope.updateTimer = null;
            scope.updateTimerTimeout && $window.clearTimeout(scope.updateTimerTimeout);
            scope.updateTimerTimeout = null;
          }
          function hide() {
            tourtip.detach();
          ...
          scope.proceed = function (jump) {
            ...
            scope.setCurrentStep(scope.getCurrentStep() + (jump || 1));
          };
        }
      } /* [/compile] */ };
    }
  ]).directive('tourClick', function() {
    return {
      restrict: 'A',
    ...
    return {
      replace: true,
      templateUrl: 'components/tour/tour.tpl.html',
      scope: false,
      restrict: 'EA',
      controller: 'TourtipController',
      link: function (scope, element, attrs) {
      }
    };
